...
bash scripts/verify_release_assets.sh "$release_dist" 0.3.6 "$release_commit"
...
if [[ -n "${ART_CARGO_AUDIT_DB:-}" ]]; then
  cargo audit --deny warnings --db "$ART_CARGO_AUDIT_DB" --no-fetch
else
