...
if [[ "$($binary --version)" != 'art 0.3.4' ]]; then
  echo 'ART installer accepts only the 0.3.4 binary' >&2
  exit 64
fi
...
test "$("$staging/art" --version)" = 'art 0.3.4'
...
echo "installed ART 0.3.4 at $art_home/bin/art"
