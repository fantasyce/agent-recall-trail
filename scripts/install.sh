...
if [[ "$($binary --version)" != 'art 0.3.5' ]]; then
  echo 'ART installer accepts only the 0.3.5 binary' >&2
  exit 64
fi
...
test "$("$staging/art" --version)" = 'art 0.3.5'
...
echo "installed ART 0.3.5 at $art_home/bin/art"
