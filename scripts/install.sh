...
if [[ "$($binary --version)" != 'art 0.3.2' ]]; then
  echo 'ART installer accepts only the 0.3.2 binary' >&2
  exit 64
fi
...
test "$("$staging/art" --version)" = 'art 0.3.2'
...
echo "installed ART 0.3.2 at $art_home/bin/art"
