...
if [[ "$($binary --version)" != 'art 0.1.0' ]]; then
  echo 'ART installer accepts only the 0.1.0 binary' >&2
  exit 64
fi
...
test "$("$staging/art" --version)" = 'art 0.1.0'
...
echo "installed ART 0.1.0 at $art_home/bin/art"
