...
with zipfile.ZipFile(d/f'agent-recall-trail_{v}.mcpb') as a:
 assert a.namelist()==sorted(a.namelist()); m=json.loads(a.read('manifest.json')); assert m['version']==v and len(m['tools'])==7 and any(t['name']=='art_knowledge_governance' for t in m['tools'])
