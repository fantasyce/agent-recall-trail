# ART v0.3.3: local knowledge governance for Codex and DSH
...
The v0.3.3 release adds the loopback governance page, persistent host-and-Agent
policy, and one exact `approve_and_publish` operation while preserving
progressive `route -> recall -> read` retrieval and four explicit modes behind
the same recall API.
